...
let scenarios = [];  // [{{team, round, delta}}]
let currentEntry = '';

function deltaBg(v) {{
  if (Math.abs(v) < 0.2) return 'transparent';
  let i = Math.min(Math.abs(v) / 8, 1);
...
function getDelta(team, rnd) {{
  const ed = DATA[currentEntry];
  if (!ed[team] || !ed[team][rnd]) return 0;
  return ed[team][rnd].w - OVERALL[currentEntry];
}}
...
function toggleScenario(team, rnd) {{
  ...
  if (idx >= 0) {{
    ...
    const delta = getDelta(team, rnd);
    const ed = DATA[currentEntry];
    scenarios.push({{ key, team, round: rnd, delta, condPct: ed[team][rnd].w }});
  }}
  updatePanel();
...
function updatePanel() {{
  ...
  // For single scenario, use exact conditional. For multiple, sum deltas (approx).
  let adjusted, totalDelta;
  if (scenarios.length === 1) {{
    adjusted = scenarios[0].condPct;
    totalDelta = adjusted - base;
  }} else if (scenarios.length > 1) {{
    totalDelta = scenarios.reduce((sum, s) => sum + s.delta, 0);
    adjusted = Math.max(0, Math.min(100, base + totalDelta));
  }} else {{
    adjusted = base;
    totalDelta = 0;
  }}
  ...
  if (scenarios.length > 0) {{
    ...
    deltaEl.className = 'delta ' + (totalDelta >= 0 ? 'up' : 'down');
  }} else {{
  ...
  if (scenarios.length === 0) {{
    listEl.innerHTML = '<span class="no-scenarios">Click any cell to see "what if this team advances?"</span>';
    clearEl.style.display = 'none';
  }} else {{
    clearEl.style.display = '';
    listEl.innerHTML = scenarios.map(s => {{
      const cls = s.delta >= 0 ? 'chip-positive' : 'chip-negative';
      const sign = s.delta >= 0 ? '+' : '';
      return `<span class="scenario-chip ${{cls}}" onclick="toggleScenario('${{s.team}}','${{s.round}}')" `
        + `title="If ${{s.team}} wins ${{RN[s.round]}}: ${{s.condPct.toFixed(1)}}% (click to remove)">`
        + `${{s.team}} wins ${{RN[s.round]}} (${{sign}}${{s.delta.toFixed(1)}}%) <span class="chip-x">\u2715</span></span>`;
    }}).join('');
    if (scenarios.length > 1) {{
      listEl.innerHTML += '<span style="color:#888;font-size:0.7rem;margin-left:4px">(approx. combined)</span>';
    }}
  }}
}}
...
function renderEntry() {{
  ...
  // Find max delta-from-baseline per team
  let teamMaxDelta = {{}};
  for (const team of TEAMS) {{
    ...
    for (const rnd of ROUNDS) {{
      if (ed[team][rnd]) {{
        let d = Math.abs(ed[team][rnd].w - base);
        if (d > maxAbs) maxAbs = d;
      }}
    }}
    teamMaxDelta[team] = maxAbs;
  }}

  const sortedTeams = Object.keys(teamMaxDelta).sort((a,b) => teamMaxDelta[b] - teamMaxDelta[a]);
  const relevantTeams = sortedTeams.filter(t => teamMaxDelta[t] >= 0.2);
  ...
  for (const team of relevantTeams) {{
    ...
    for (const rnd of ROUNDS) {{
      if (td[rnd]) {{
        const delta = td[rnd].w - base;
        if (Math.abs(delta) > Math.abs(peak)) peak = delta;
        const cls = delta > 0.2 ? 'positive' : delta < -0.2 ? 'negative' : 'neutral';
        const arrow = delta > 0.2 ? '\u25B2' : delta < -0.2 ? '\u25BC' : '';
        const key = scenarioKey(team, rnd);
        const sel = scenarios.some(sc => sc.key === key) ? ' selected' : '';
        html += `<td class="clickable ${{cls}}${{sel}}" style="background:${{deltaBg(delta)}}" `;
        html += `data-key="${{key}}" onclick="toggleScenario('${{team}}','${{rnd}}')" `;
        html += `title="If ${{team}} wins ${{RN[rnd]}}: ${{td[rnd].w.toFixed(1)}}%">`;
        html += `${{arrow}}${{delta > 0 ? '+' : ''}}${{delta.toFixed(1)}}%</td>`;
      }} else {{
        html += '<td class="neutral">\u2014</td>';
      }}
    }}
    const pcls = peak > 0.2 ? 'positive' : peak < -0.2 ? 'negative' : 'neutral';
    html += `<td class="${{pcls}}" style="font-weight:700">${{peak > 0 ? '+' : ''}}${{peak.toFixed(1)}}%</td>`;
    html += '</tr>';
  }}

  html += '</tbody></table>';
  html += '<p class="help">Each cell = change in your win% if that team advances. Click to add scenarios.</p>';
